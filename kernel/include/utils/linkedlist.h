...
#define list_entry(ptr, type) ((type*) ptr->data)

#define list_first_entry(list, type) ((type*) (list)->next->data)

#define list_last_entry(list, type) ((type*) (list)->prev->data)

#define list_for_each(iter, pos, list) \
    for (iter = (list)->next, pos = (typeof(pos)) iter->data; iter != (list); \
         iter = iter->next, pos = (typeof(pos)) iter->data)

#define list_for_each_entry(pos, list) \
    pos = (typeof(pos)) (list)->next->data; \
    for (list_t* __iter = (list)->next; __iter != (list); \
         __iter = __iter->next, pos = (typeof(pos)) __iter->data)

#define list_for_each_safe(iter, n, list) \
    for (iter = (list)->next, n = iter->next; iter != (list); iter = n, n = iter->next)

#define list_for_each_entry_rev(iter, pos, list) \
    for (iter = (list)->prev, pos = (typeof(pos)) iter->data; iter != (list); \
         iter = iter->prev, pos = (typeof(pos)) iter->data)
